...
def is_cuda_cv(): # 1 for CUDA, 0 for CPU
	try:
...
def sendDataToServer(x, y, Vx, Vy):	
	...
	print("CUDA Enabled: {} version: {}",is_cuda_cv(), cv2.__version__)	
...
def processImage(undistorted_frame):
	...
	ballLower0 = np.array([62, 36, 99], np.uint8)
	ballUpper0 = np.array([98, 113, 199], np.uint8)
